...
public class MultiServer : MonoBehaviour
{
    ...
    private void ClientListenerThread(object nThread)
    {
        ...
        SendMessageToClient(JsonUtility.ToJson(baseRequest_NC), threadKey);
        ...
        while (handler.Connected)
        {
            ...
            Debug.Log(String.Format("<<<Thread {0}>>>: Text received : {1}", threadKey, data));
            if (data.IndexOf("\0") > -1)
            {
                MessageReceived(Encoding.ASCII.GetString(Convert.FromBase64String(data)), threadKey);
                data = null;
            }
        }
    ...
    void messageReceivedListener(string _message, int clientID)
    {
        ...
        if ((MultiPossibleRequest)JsonUtility.FromJson<MultiBaseRequest>(_message).RequestType == MultiPossibleRequest.MultiSceneObjects)
        {
            ...
            SendMessageToClient(_message, mostRecentConnection);
        }
